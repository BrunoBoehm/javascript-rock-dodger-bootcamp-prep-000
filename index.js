...
function createRock(x) {
  ...
  function moveRock() {
    ...
    // if we did only top + 2 we would leave the top variable always at the same value
    // this is the var top that is incremented, not to be mistaken with the .top
...
function moveDodgerLeft() {
   ...
   */
   ...
   function step() {
     leftDodger -= 4;
     if (leftDodger > 0) {
       DODGER.style.left = `${leftDodger}px`
...
function moveDodgerRight() {
   ...
   */
   ...
   function step() {
     rightDodger += 4;
     if (rightDodger < 360) {
       DODGER.style.right = `${rightDodger}px`
       window.requestAnimationFrame(step);
     }
   }
   window.requestAnimationFrame(step);
